...
- **최근 거래일 예측 그래프 (24.09.11 ~ 24.10.30)**  
  ...
  - **단기적인 추세 일치**  
    일부 테스트 기간 동안 파란 실선(실제 시가)과 주황 점선(예측 시가)이 대체로 겹치는 경향을 보여 모델이 단기 트렌드를 비교적 잘 포착했음이 드러남
  
  - **휴장일 연속성 유지**
    주말·공휴일로 인한 거래일 공백 구간 이후에도 예측 곡선이 자연스럽게 이어져 데이터 간격 불연속에도 일관된 성능을 보임
  
  - **보수적 예측 경향 (개선 필요)**  
    예측 곡선이 실제 곡선 대비 변동 폭을 다소 축소해서 그려져 급격한 등락 시에도 딱히 다이나믹하지 않게 관측됨
