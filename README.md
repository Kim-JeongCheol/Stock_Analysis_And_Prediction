...
- **평가지표**
  ...
  **MSE (Mean Squared Error)**  
  예측값과 실제값의 차이를 제곱해 평균한 값으로, 전체적인 예측 오차의 크기를 나타냅니다.  
  값이 작을수록 모델 예측이 실제값에 가깝다는 의미입니다.

  **R² (결정 계수)**  
  전체 시가 변동 중 모델이 설명해 낸 비율입니다.  
  1에 가까울수록 데이터 패턴을 잘 포착한 고성능 모델임을 보여줍니다.

  **MAPE (Mean Absolute Percentage Error)**  
  예측 오차를 실제값 대비 백분율로 환산해 평균한 지표입니다.  
  “평균적으로 실제 시가 대비 몇 % 어긋났는지”를 직관적으로 파악할 수 있습니다.
