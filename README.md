...
- **전체 예측 그래프 (23.08.16 ~ 24.10.30)**  
  ![전체 예측](images/predictions.png)
   -**전반적인 추세 일치**  
    전체 테스트 기간(2023-09 ~ 2024-11) 동안 파란 선(실제 시가)과 주황 점선(예측 시가)이 거의 겹쳐, 모델이 **장기·단기 트렌드를 모두 잘 포착**했음을 보여줍니다.

  - **강세장 구간(2024-03 ~ 2024-07)**  
    급격한 상승 구간에서도 예측 시가가 **상승 모멘텀을 정확히 좇아갔**습니다.  
    다만, 봉우리 직전·직후에 약간의 **lag**(예측 지연)가 발생해 미세한 과소/과대 예측이 관찰됩니다.

  - **변동성 확대 구간**  
    단기 급등락(작은 스파이크) 구간에서도 예측 곡선이 실제 곡선을 **빠르게 따라붙어**,  
    예측 반응 속도가 빠름을 알 수 있습니다.

  - **하락장 구간(2024-09 이후)**  
    지속적인 하락세 구간에서도 예측 시가가 **낙폭과 타이밍을 정확히 반영**해,  
    **하락장에서도 높은 신뢰도**를 제공합니다.

  - **잔차(Residual) 수준**  
    평균 잔차 규모는 RMSE ≈ 927원, MAPE = 0.94%로,  
    y축(약 55,000~90,000원) 범위 대비 **오차 폭이 매우 작아** 시각적으로 거의 구분이 불가능합니다.
