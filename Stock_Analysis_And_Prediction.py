...
class preprocessor:
    ...
    @staticmethod
    def create_sequences(data: np.ndarray, seq_len: int = 7, pred_steps: int = 1):
        # LSTM 입력용 시퀀스(X)와 라벨(y) 생성
        # data : 스케일링된 numpy 배열
        # seq_len : 과거 시퀀스 길이 (일주일)
...
if __name__ == '__main__':
    ...
    # 모델 학습
    model = LSTMModel(seq_len=7, n_features=scaled.shape[1])
    ...
    # 예측 결과 시각화
    Visualizer.plot_predictions(dates[n_train+7:], testY_orig, y_pred_orig, zoom_len=50)
    ...
    print(f"[Result] MSE: {mse:.4f}, R2: {r2:.4f}, MAPE: {mape:.2%}")
